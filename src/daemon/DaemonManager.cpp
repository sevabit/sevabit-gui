...
{
    // prepare command line arguments and pass to sevabitd
    ...
    qDebug() << "starting sevabitd " + m_sevabitd;
    ...
    // Start sevabitd
    bool started = m_daemon->startDetached(m_sevabitd, arguments);
    ...
    while(true && !m_app_exit) {
        ...
        if(running(nettype)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM sevabitd.exe");
#else
                QProcess::execute("pkill sevabitd");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./sevabitd status` returns BUSY when syncing.
...
bool DaemonManager::sendCommand(const QString &cmd, NetworkType::Type nettype, QString &message) const
{
    ...
    p.start(m_sevabitd, external_cmd);
    ...
    : QObject(parent)
{
    ...
    // Platform depetent path to sevabitd
#ifdef Q_OS_WIN
    m_sevabitd = QApplication::applicationDirPath() + "/sevabitd.exe";
#elif defined(Q_OS_UNIX)
    m_sevabitd = QApplication::applicationDirPath() + "/sevabitd";
#endif

    if (m_sevabitd.length() == 0) {
